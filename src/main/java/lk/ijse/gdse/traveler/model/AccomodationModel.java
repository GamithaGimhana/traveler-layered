...
public class AccomodationModel {
    public String getNextAccomodationId() throws SQLException {
        ResultSet rst = CrudUtil.execute("select accommodation_id from accommodation order by accommodation_id desc limit 1");
    ...
    public ArrayList<AccomodationDTO> getAllAccommodations() throws SQLException {
        ResultSet rst = CrudUtil.execute("select * from accommodation");
    ...
    public boolean deleteAccommodation(String accommodationId) throws SQLException {
        return CrudUtil.execute("delete from accommodation where accommodation_id=?", accommodationId);
    }
}
